...
import { analyzePageSpeed } from '../../../services/geminiService';
...
export function PageSpeedAnalyzer() {
  ...
  const { result, isLoading, error, generate } = useToolResults<string>({
    ...
    generateFn: () => analyzePageSpeed(url),
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold mb-2">PageSpeed AI Analyzer</h2>
        <p className="text-sm text-muted-foreground">
          Analyze tech stack and get optimization recommendations for your website.
        </p>
      </div>
